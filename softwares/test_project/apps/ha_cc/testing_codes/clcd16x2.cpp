...
/* Definitions */
#if CLCD16x2_DEBUG
#define CLCD16x2_PRINTF(...) printf(__VA_ARGS__)
#else
#define CLCD16x2_PRINTF(...)
#endif

/*--------------------------- Config interface -------------------------------*/
GPIO_TypeDef *RS_port = GPIOB;
const uint16_t RS_pin = GPIO_Pin_11;
GPIOMode_TypeDef RS_mode[] = {GPIO_Mode_Out_PP, GPIO_Mode_IN_FLOATING};
GPIOSpeed_TypeDef RS_speed = GPIO_Speed_2MHz;
const uint32_t RS_RCC = RCC_APB2Periph_GPIOB;

GPIO_TypeDef *EN_port = GPIOB;
const uint16_t EN_pin = GPIO_Pin_10;
GPIOMode_TypeDef EN_mode[] = {GPIO_Mode_Out_PP, GPIO_Mode_IN_FLOATING};
GPIOSpeed_TypeDef EN_speed = GPIO_Speed_2MHz;
const uint32_t EN_RCC = RCC_APB2Periph_GPIOB;

GPIO_TypeDef *BL_port = GPIOC;
const uint16_t BL_pin = GPIO_Pin_6;
GPIOMode_TypeDef BL_mode[] = {GPIO_Mode_Out_PP, GPIO_Mode_IN_FLOATING};
GPIOSpeed_TypeDef BL_speed = GPIO_Speed_2MHz;
const uint32_t BL_RCC = RCC_APB2Periph_GPIOC;

GPIO_TypeDef *DB4_port = GPIOB;
const uint16_t DB4_pin = GPIO_Pin_12;
GPIOMode_TypeDef DB4_mode[] = {GPIO_Mode_Out_PP, GPIO_Mode_IN_FLOATING};
GPIOSpeed_TypeDef DB4_speed = GPIO_Speed_2MHz;
const uint32_t DB4_RCC = RCC_APB2Periph_GPIOB;

GPIO_TypeDef *DB5_port = GPIOB;
const uint16_t DB5_pin = GPIO_Pin_13;
GPIOMode_TypeDef DB5_mode[] = {GPIO_Mode_Out_PP, GPIO_Mode_IN_FLOATING};
GPIOSpeed_TypeDef DB5_speed = GPIO_Speed_2MHz;
const uint32_t DB5_RCC = RCC_APB2Periph_GPIOB;

GPIO_TypeDef *DB6_port = GPIOB;
const uint16_t DB6_pin = GPIO_Pin_14;
GPIOMode_TypeDef DB6_mode[] = {GPIO_Mode_Out_PP, GPIO_Mode_IN_FLOATING};
GPIOSpeed_TypeDef DB6_speed = GPIO_Speed_2MHz;
const uint32_t DB6_RCC = RCC_APB2Periph_GPIOB;

GPIO_TypeDef *DB7_port = GPIOB;
const uint16_t DB7_pin = GPIO_Pin_15;
...
clcd16x2::~clcd16x2(void) {
	...
	/* Init RS, RW, EN */
	gpio_init_s.GPIO_Mode = RS_mode[in];
	...
	gpio_init_s.GPIO_Mode = EN_mode[in];
	...
	gpio_init_s.GPIO_Mode = BL_mode[in];
	...
	gpio_init_s.GPIO_Mode = DB6_mode[in];
	...
	gpio_init_s.GPIO_Mode = DB5_mode[in];
	...
	gpio_init_s.GPIO_Mode = DB4_mode[in];
...
/*----------------------------------------------------------------------------*/
int16_t clcd16x2::set_cursor(uint8_t line, uint8_t pos) {
	uint8_t ddram_addr = 0;

	/* check line and pos */
	if ((line < 1) || (line > 2)) {
		return clcd16x2_ns::failed;
	}
	...
	return clcd16x2_ns::successful;
}

/*----------------------------------------------------------------------------*/
void clcd16x2::clear(void) {
	/* send clear command */
...
/*----------------------------------------------------------------------------*/
void clcd16x2::home_woshift(void) {
	set_cursor(1, 1);
}

/*----------------------------------------------------------------------------*/
void clcd16x2::putc(int8_t a_char) {
	uint8_t tab_pos;
...
/*----------------------------------------------------------------------------*/
int16_t clcd16x2::puts(int8_t* a_string) {
	int16_t num_char = 0;
...
/*----------------------------------------------------------------------------*/
int16_t clcd16x2::printf(const char *format, ...) {
	int8_t buffer[20*4];
	int16_t retval;
	va_list args;
...
/*----------------------------------------------------------------------------*/
void clcd16x2::gen_pattern(const uint8_t *pattern, uint8_t addr) {
	uint16_t count;
...
/*----------------------------------------------------------------------------*/
void clcd16x2::set_backlight(bool backlight){
	if(backlight == 0) {
		/* Turn off back light */
...
void clcd16x2::send_byte(uint8_t RS, uint8_t RW, uint8_t D7D0) {
...
/*----------------------------------------------------------------------------*/
void clcd16x2::send_4bit(uint8_t RS, uint8_t RW, uint8_t D7D4) {
...
/*----------------------------------------------------------------------------*/
void clcd16x2::send_byte_wowait(uint8_t RS, uint8_t RW, uint8_t D7D0) {
	RS ? GPIO_SetBits(RS_port, RS_pin) : GPIO_ResetBits(RS_port, RS_pin);
...
/*----------------------------------------------------------------------------*/
void clcd16x2::init_gpios(void) {
	GPIO_InitTypeDef gpio_init_s;

	/* Init RS, RW, EN */
	gpio_init_s.GPIO_Mode = RS_mode[out];
	gpio_init_s.GPIO_Speed = RS_speed;
	gpio_init_s.GPIO_Pin = RS_pin;
	...
	gpio_init_s.GPIO_Mode = EN_mode[out];
	gpio_init_s.GPIO_Speed = EN_speed;
	gpio_init_s.GPIO_Pin = EN_pin;
	...
	gpio_init_s.GPIO_Mode = BL_mode[out];
	gpio_init_s.GPIO_Speed = BL_speed;
	gpio_init_s.GPIO_Pin = BL_pin;
	...
	gpio_init_s.GPIO_Mode = DB6_mode[out];
	gpio_init_s.GPIO_Speed = DB6_speed;
	gpio_init_s.GPIO_Pin = DB6_pin;
	...
	gpio_init_s.GPIO_Mode = DB5_mode[out];
	gpio_init_s.GPIO_Speed = DB5_speed;
	gpio_init_s.GPIO_Pin = DB5_pin;
	...
	gpio_init_s.GPIO_Mode = DB4_mode[out];
	gpio_init_s.GPIO_Speed = DB4_speed;
	gpio_init_s.GPIO_Pin = DB4_pin;
...
/*----------------------------------------------------------------------------*/
void clcd16x2::delay_us(uint32_t n) {
	testing_delay_us(n);
}
